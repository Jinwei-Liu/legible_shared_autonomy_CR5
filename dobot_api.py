    ...
    def __init__(self, ip, port, *args):
        ...
        if self.port == 29999 or self.port == 30004 or self.port == 30005:
            try:
                ...
                print(f"Warning: Failed to connect to {self.ip}:{self.port} - {e}")
...
class DobotApiDashboard(DobotApi):
    ...
    def MovL_NoWait(self, a1, b1, c1, d1, e1, f1, coordinateMode, user=-1, tool=-1, a=-1, v=-1, speed=-1, cp=-1, r=-1):
        """
        MovL without waiting for response - send only
        """
        string = ""
        if coordinateMode == 0:
            string = "MovL(pose={{{:f},{:f},{:f},{:f},{:f},{:f}}}".format(a1, b1, c1, d1, e1, f1)
        elif coordinateMode == 1:
            string = "MovL(joint={{{:f},{:f},{:f},{:f},{:f},{:f}}}".format(a1, b1, c1, d1, e1, f1)
        else:
            print("coordinateMode param is wrong")
            return
        params = []
